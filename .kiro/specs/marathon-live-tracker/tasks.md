...
- [x] 1. Create project structure and HTML entry point
  - [x] 1.1 Create `index.html` with full-screen map container, Mapbox GL JS/CSS CDN links, and ES module script imports
    - Include a `<div id="map">` that fills the viewport
    - Link `style.css` and load `app.js` as a module
    - _Requirements: 1.1_
  - [x] 1.2 Create `style.css` with full-screen map styles and UI overlay styles
...
- [x] 2. Implement course data module
  - [x] 2.1 Create a Node.js conversion script (`scripts/convert-gpx.js`) that parses `course/gpx_20250421_id10253_race1_20250406001335.gpx`, extracts all `<trkpt>` lat/lon pairs, converts to `[lng, lat]` GeoJSON order, computes the bounding box, and writes the result into `course-data.js` as a static ES module
  ...
  - [x] 2.2 Run the conversion script and verify `course-data.js` exports `getCourseGeoJSON()`, `getCourseLengthMiles()`, and `getCourseBounds()` correctly
...
- [x] 3. Implement pacing plan and position calculator modules
  - [x] 3.1 Create `pacing-plan.js` exporting `getPacingPlan()` and `buildCumulativeTimeTable()`
  ...
  - [x] 3.2 Write property test: Cumulative time table correctness (Property 1)
  ...
  - [x] 3.3 Write property test: Pacing plan structural validity (Property 4)
  ...
  - [x] 3.4 Create `position-calculator.js` exporting `getDistanceAtTime()` and `interpolatePosition()`
  ...
  - [x] 3.5 Write property test: Distance-at-time monotonicity and clamping (Property 2)
  ...
  - [x] 3.6 Write property test: Geographic interpolation lies on polyline (Property 3)
...
- [x] 4. Checkpoint - Verify core computation modules
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Implement cookie store and UI controller
  - [x] 5.1 Create `cookie-store.js` exporting `getStartTime()`, `setStartTime()`, and `clearStartTime()`
  ...
  - [x] 5.2 Write property test: Cookie store round trip (Property 5)
  ...
  - [x] 5.3 Create `ui-controller.js` exporting `initUI()`, `validateStartTime()`, `showError()`, `clearError()`
  ...
  - [x] 5.4 Write property test: Invalid start time rejection (Property 6)
...
- [x] 6. Implement map controller
  - [x] 6.1 Create `map-controller.js` exporting `initMap()`, `updateRunnerPosition()`, `toggleMapStyle()`, `getActiveStyle()`
  ...
  - [x] 6.2 Write property test: View toggle round trip (Property 7)
...
- [x] 7. Wire everything together in app.js
  - [x] 7.1 Create `app.js` as the main entry point that orchestrates all modules
    - Import all modules
...
- [x] 8. Implement debug mode UI and integration
  - [x] 8.1 Add hamburger menu icon to `index.html` and style it in `style.css`
  ...
  - [x] 8.2 Add debug mode toggle and time slider to the hamburger menu
  ...
  - [x] 8.3 Update `ui-controller.js` to export `isDebugMode()` and `getSliderValue()`
    - `isDebugMode()` returns the current debug mode state
    - `getSliderValue()` returns the current slider value (0–100)
    - _Requirements: 9.4, 9.7_
  - [x] 8.4 Write property test: Debug mode slider-to-elapsed-time mapping (Property 8)
  ...
  - [x] 8.5 Write property test: Debug mode toggle restores real-time behavior (Property 9)
